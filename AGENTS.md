...
Weakspot uses persisted mistyped words for the selected language as a direct drill; rank by failure/success ratio, then absolute failures, and use as many as the current test requires. Include weakspot results in word success/failure stats, but not character or mistake-type stats.
